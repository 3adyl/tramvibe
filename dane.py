...
class Przystanek:
    ...
    def dystans(self,szer,dlug):
        return round((round((float(self.szer_geo)-szer),6)**2 + (round((float(self.dlug_geo)-dlug),6)**2))**(1/2),6)

    def rozklad_full(self, **kwargs):
        dostepne_linie = requests.get(f"https://api.um.warszawa.pl/api/action/dbtimetable_get?id=88cd555f-6f31-43ca-9de4-66c479ad5942&busstopId={self.zespol}&busstopNr={self.slupek}&apikey={config['API_KEY']}")
        z = []
        for i in dostepne_linie.json()['result']:
            dane_rozkladu = requests.get(f"https://api.um.warszawa.pl/api/action/dbtimetable_get?id=e923fa0e-d96c-43f9-ae6e-60518c9f3238&busstopId={self.zespol}&busstopNr={self.slupek}&line={i['values'][0]['value']}&apikey={config['API_KEY']}")
    ...
    def rozklad_wyswietlacz_now(self):
        ...
        z = [i for i in z if datetime.strptime(i[2], '%Y-%m-%d %H:%M:%S') >= datetime.now()]
        for i in z:
            i[2] = str(int(round(-(datetime.now() - datetime.strptime(i[2], '%Y-%m-%d %H:%M:%S')).total_seconds()/60,0)))+" min"
        z.append(['','KONIEC KURSOWANIA',''])
        return z
